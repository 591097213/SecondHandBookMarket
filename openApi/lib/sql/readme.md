...
如记录中有自增字段，例如id，则插入成功后返回此字段，失败返回0。如果没有自增字段，则插入成功返回1，失败返回0。
```php
...
Insert with on duplicate key update（重复插入时更新）
```php
example1:
$data = Array ("login" => "admin",
               "firstName" => "John",
...
$updateColumns = Array ("firstName","lastName");
$db->onDuplicate($updateColumns);
$id = $db->insert ('users', $data);
//设表格含有id，login，firstName，lastName字段，id为primary key，且自增长，login为unique
//上述语句等效于：
//INSERT INTO users ("login","firstName","lastName") VALUES ("admin","John",'Doe') 
//ON DUPLICATE KEY UPDATE "firstName" = "John","lastName" = 'Doe';  
//系统会以unique字段login为依据，若重复则更新firstName,lastName
//等效于：  
//UPDATE table SET "firstName" = "John","lastName" = 'Doe' WHERE "login" => "admin";  

//若id与login字段同时出现在$data字段中，则等效于
//UPDATE table SET "firstName" = "John","lastName" = 'Doe' WHERE "login" => "admin" OR "id"="xxx";
```
...
```php
// WRONG,表示lastLogin字段的值为createdAt
$db->where ('lastLogin', 'createdAt');
// CORRECT，表示lastLogin字段与createAt字段相等
$db->where ('lastLogin = createdAt');
$results = $db->get ('users');
