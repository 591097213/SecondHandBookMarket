...
//需要先手动建立testdb数据库
$db = new Mysqlidb('localhost', 'root', '123456789', 'testdb');
$prefix = 't_'; //表前缀
$db->setPrefix($prefix);
dbObject::autoload("models"); //加载表关系定义类
...

{
    ...
    $q = "CREATE TABLE $name (id INT(9) UNSIGNED PRIMARY KEY AUTO_INCREMENT"; //创建id，设为自增长
    foreach ($data as $k => $v) { //附加其他字段
        $q .= ", $k $v";
    }
    $q .= ")";
    $db->rawQuery($q);
...
foreach ($tables as $name => $fields) {
    $db->rawQuery("DROP TABLE " . $prefix . $name); //若已有表，则丢弃
    createTable($prefix . $name, $fields); //创建user和products两张新表
}

foreach ($data as $name => $datas) {
    foreach ($data[$name] as $userData) {
        $obj = new $name($userData); //存入数据
        $id = $obj->save();
        if ($obj->errors) {
            echo "errors:";
            print_r($obj->errors);
...
//testArrayBuilder
$products = product::ArrayBuilder()->get(2); //produce是models里的类名
